...
def d1_distance(delta: np.ndarray) -> List[float]:
    ...
    return np.sum(delta != 0, axis=1, dtype=np.float64).tolist()
...
def d2_distance(delta: np.ndarray) -> List[float]:
    ...
    return np.sum(np.abs(delta), axis=1, dtype=np.float64).tolist()
...
def d3_distance(delta: np.ndarray) -> List[float]:
    ...
    List[float]
    """
    return np.sum(np.square(np.abs(delta)), axis=1, dtype=np.float64).tolist()
...
def get_delta(instance: np.ndarray, cf: np.ndarray) -> np.ndarray:
    ...
    # print(type(cf))
    # print(type(instance))
    return cf.astype(np.float64) - instance.astype(np.float64)

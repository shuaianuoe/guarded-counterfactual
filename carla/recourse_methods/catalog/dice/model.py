...
from carla.recourse_methods.processing import check_counterfactuals, encode_feature_names
...
class Dice(RecourseMethod):
    ...
    def __init__(self, mlmodel: MLModel, hyperparams: Dict[str, Any]) -> None:
        ...
        # self._dice = dice_ml.Dice(self._dice_data, self._dice_model, method="genetic")
        self._num = checked_hyperparams["num"]
        self._desired_class = checked_hyperparams["desired_class"]
        # print("_desired_class:", self._desired_class)
        self._post_hoc_sparsity_param = checked_hyperparams["posthoc_sparsity_param"]
        
        
        # added by shuai
        self._immutables = encode_feature_names(
            self._model.data.immutables, self._model.feature_input_order
        )
        self._mutables = [
            feature
            for feature in self._model.feature_input_order
            if feature not in self._immutables
        ]
        
        # print("self._mutables",self._mutables)
    ...
    def get_counterfactuals(self, factuals: pd.DataFrame) -> pd.DataFrame:
        ...
        # print(df_cfs)
        # If in order to generate a counterfactual of counterfactual, it must be commented out

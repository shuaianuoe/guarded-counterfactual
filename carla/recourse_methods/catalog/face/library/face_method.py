...
def graph_search(
    ...
    # added by shuai
    # immutable_constraint_matrix1 = np.array([])
    # immutable_constraint_matrix2 = np.array([])
    
    # this means keys_immutable cannot be empty
    # print(keys_immutable)
    ...
    if desired_class == 1:
        ...
        y_predicted = model.predict_proba(data.values)
        y_predicted = np.argmax(y_predicted, axis=1)
        y_positive_indeces = np.where(y_predicted == 0)
    else:
        raise ValueError("The desired_class can only be 0 and 1")
...
def choose_random_subset(data, frac, index):
    ...
    number_samples = np.rint(frac * data.values.shape[0]).astype(np.int64)
    list_to_choose = (

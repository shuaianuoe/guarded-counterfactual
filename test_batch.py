...
from batch_alg import greedy_maximum_coverage, greedy_set_cover, find_opt_min_cover, find_opt_max_coverage
import sys
...
for i, (index, fact_row) in enumerate(factuals.iterrows()):
    ...
    if selected_df.shape[0] < cfs_num: 
        ...
        continue   
    proceed_total_time += time.time()-start_time
    #%% 
    start_time = time.time()
    subset_dict = {}
    ...
    if cf_method in ['opt']:
        mc_res = find_opt_max_coverage(universe, subset_copy_dict, cfs_num)
    else:
    ...
    if sDBC == True:
        subset_copy_dict = subset_dict.copy()
        if cf_method in ['opt']:
            sc_res = find_opt_min_cover(universe, subset_copy_dict, feat_num)
        else:

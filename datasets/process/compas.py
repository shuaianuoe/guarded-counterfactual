...
def get_and_preprocess_compas_data():
    ...
    compas_df = pd.read_csv("compas-scores-2years.csv", index_col=0)
    ...
    X["score"] = np.array(
        [
            NEGATIVE_OUTCOME if score in ["High", "Medium"] else POSITIVE_OUTCOME # added by shuai
            # NEGATIVE_OUTCOME if score in ["High"] else POSITIVE_OUTCOME 
